...
#'
#' @usage
#' adaptspec2(nloop, nwarmup, nexp_max, x,
...
#' model1 <- adaptspec2(nloop = 2000, nwarmup = 500,
...
#' model2 <- adaptspec2(nloop = 400, nwarmup = 100,
...
adaptspec2 <- function(
  ...
  results <- .adaptspec(
    nloop, nwarmup, nexp_max, x,
    tmin, sigmasqalpha, tau_prior_a, tau_prior_b, tau_up_limit, prob_mm1, nbasis,
    nexp_start, show_progress
  )

  results$beta <- aperm(results$beta, c(3, 1, 2))
  results$tau_squared <- aperm(results$tau_squared, c(2, 1))
  results$cut_point <- aperm(results$cut_point, c(2, 1))
